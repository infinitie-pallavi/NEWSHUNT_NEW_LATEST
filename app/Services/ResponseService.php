...
const AT_LINE = ' At Line : ';
const NOT_ENOUGH_PERMISSION = "You Don't have enough permissions";
class ResponseService {
    ...
    public static function noPermissionThenRedirect($permission) {
        if (!Auth::user()->can($permission)) {
            return redirect(route('dashboard'))->withErrors([
                'message' => trans(NOT_ENOUGH_PERMISSION)
            ])->send();
        }
        return true;
    }
    ...
    public static function noPermissionThenSendJson($permission) {
        if (!Auth::user()->can($permission)) {
            self::errorResponse(NOT_ENOUGH_PERMISSION);
        }
        return true;
    }
     ...
     * @return void
     */
    public static function successResponse(string|null $message = "Success", $data = null, array $customData = array(), $code = null) {
     ...
     * @return void
     */
    public static function errorResponse(string $message = ERROR_MESSAGE, $data = null, $code = null, $e = null) {
        response()->json([
            'error'   => true,
            'message' => trans($message),
            'data'    => $data,
            'code'    => $code ?? config('constants.RESPONSE_CODE.EXCEPTION_ERROR'),
            'details' => (!empty($e) && is_object($e)) ? $e->getMessage() . ' --> ' . $e->getFile() . AT_LINE . $e->getLine() : ''
        ])->send();
        exit();
    }
     ...
     */
    public static function errorRedirectResponse(string|array $message = ERROR_MESSAGE, $url = 'back', $input = null) {
     ...
     * @return void
     */
    public static function warningResponse(string $message = ERROR_MESSAGE, $data = null, $code = null) {
        response()->json([
            'error'   => false,
            'warning' => true,
            'code'    => $code,
     ...
     * @return void
     */
    public static function validationError(string $message = ERROR_MESSAGE, $data = null) {
     ...
     * @return void
     */
    public static function validationErrorRedirect(string $message = ERROR_MESSAGE) {
     ...
     * @return void
     */
    public static function logErrorResponse(Throwable|Exception $e, string $logMessage = ' ', string $responseMessage = ERROR_MESSAGE, bool $jsonResponse = true) {
        Log::error($logMessage . ' ' . $e->getMessage() . '---> ' . $e->getFile() . AT_LINE . $e->getLine() . "\n\n" . request()->method() . " : " . request()->fullUrl() . "\nParams : ", request()->all());
     ...
     * @param bool $jsonResponse
     */
    public static function logErrorRedirect($e, string $logMessage = ' ', bool $jsonResponse = true) {
        Log::error($logMessage . ' ' . $e->getMessage() . '---> ' . $e->getFile() .AT_LINE . $e->getLine());
        if ($jsonResponse && config('app.debug')) {
            throw $e;
        }
