                            ...
                            <div class="panel vstack md:items-center justify-center h-screen overflow-hidden">
                                ...
                                <div class="panel py-4 px-2">
                                    <div class="panel vstack gap-3 w-100 sm:w-350px mx-auto text-center" data-anime="targets: >*; translateY: [24, 0]; opacity: [0, 1]; easing: easeInOutExpo; duration: 750; delay: anime.stagger(100);">
                                        <h1 class="h4 sm:h3">User Login</h1>
                                        ...
                                        <form method="POST" action="{{ route('user.login') }}" class="vstack gap-2">
                                            @csrf
                                            <!-- Email Input -->
                                            <input class="form-control form-control-sm h-40px w-full fs-6 bg-white dark:bg-opacity-0 dark:text-white dark:border-gray-300 dark:border-opacity-30"type="email" name="email" placeholder="Your email" required value="{{ old('email') }}">
                                            @error('email')
                                                <div class="text-danger">{{ $message }}</div>
                                            @enderror
                                        
                                            <!-- Password Input -->
                                            <input class="form-control form-control-sm h-40px w-full fs-6 bg-white dark:bg-opacity-0 dark:text-white dark:border-gray-300 dark:border-opacity-30" type="password" name="password" placeholder="Password" autocomplete="current-password" required>
                                            @error('password')
                                                <div class="text-danger">{{ $message }}</div>
                                            @enderror
                                            ...
                                            <div class="hstack justify-between text-start">
                                                <div class="form-check text-start">
                                                    <input id="form_remember_me" class="form-check-input rounded bg-white dark:bg-opacity-0 dark:text-white dark:border-gray-300 dark:border-opacity-30" type="checkbox" name="remember">
                                                    <label for="form_remember_me" class="hstack justify-between form-check-label fs-6">Remember me?</label>
                                                </div>
                                                <a href="{{ route('password.request') }}" class="uc-link fs-6">Forgot password</a>
                                            </div>
                                        
                                            <!-- Submit Button -->
                                            <button class="btn btn-primary btn-sm mt-1" type="submit">Log in</button>
                                        </form>
