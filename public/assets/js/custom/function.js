document.addEventListener('DOMContentLoaded', function() {
    ...
    if (userEditModal) {
        userEditModal.addEventListener('show.bs.modal', function (event) {
            ...
            const userProfile = button.getAttribute('data-profile');

            const modalForm = userEditModal.querySelector('form');
            if (modalForm) {
                modalForm.action = `users/${userId}`;

                modalForm.querySelector('input[name="name"]').value = userName;
                modalForm.querySelector('input[name="email"]').value = userEmail;
                modalForm.querySelector('input[name="phone"]').value = userPhone;
                modalForm.querySelector('select[name="status"]').value = userStatus;
                modalForm.querySelector('img#user-profile-privew').src = userProfile;
            }
        });
    }
});
